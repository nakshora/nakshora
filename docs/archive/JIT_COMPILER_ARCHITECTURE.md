...
async function scanSourceFiles(patterns: string[], ignorePatterns?: string[]): Promise<string[]> {
...
function parseClass(className: string): ClassMatch | null {
  ...
  const variants = variantPart
    ? variantPart
        .split(':')
        .filter(Boolean)
        .map((v) => ({ name: v, type: getVariantType(v) }))
    : [];
...
function applyVariant(rule: string, variant: VariantMatch, theme: ThemeConfig): string {
  switch (variant.type) {
    case 'pseudo-class': {
...
async function compile(options: {
  ...
  const contentPatterns = Array.isArray(options.content) ? options.content : [options.content];
...
class CSSCache {
  ...
  // Store in cache
  set(content: string, config: NakshoraConfig, css: string, dependencies: string[]): void {
    const key = this.hash(content, config);
...
class PerformanceOptimizer {
  ...
  async compileInParallel(
    ...
    const results = await Promise.all(batches.map((batch) => compiler(batch)));
    return results.join('\n');
  }
}
```
