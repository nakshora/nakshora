...
async function resolveContent(content, baseDir) {
  ...
  for (const entry of entries) {
    ...
    if (entry.includes("*") || entry.includes("{") || entry.includes("[")) {
      globs.push((0, import_node_path.resolve)(baseDir, entry));
    } else {
      try {
        chunks.push((0, import_node_fs.readFileSync)((0, import_node_path.resolve)(baseDir, entry), "utf-8"));
      } catch {
        chunks.push(entry);
      }
    }
  ...
  if (globs.length > 0) {
    ...
    for (const file of files) {
      try {
        chunks.push((0, import_node_fs.readFileSync)((0, import_node_path.resolve)(baseDir, file), "utf-8"));
      } catch {
      }
    }
  }
