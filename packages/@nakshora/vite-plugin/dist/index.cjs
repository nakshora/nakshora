...
var palette_default2 = defaultColors2;
var defaultTheme2 = {
  colors: palette_default2,
  spacing: {
    0: "0",
    px: "1px",
    0.5: "0.125rem",
...
function spacingKeys2(spacing) {
  return spacing ? Object.keys(spacing) : [];
}
function colorEntries2(colors) {
  const out = [];
  if (!colors) return out;
  for (const [name, value] of Object.entries(colors)) {
    if (typeof value === "string") {
...
function buildUtilityList2(theme) {
  const rules = [];
  const add = (className, group, decls, category, description) => {
    rules.push({ class: className, group, decls, category, description });
  };
  const spacing = theme.spacing;
  const sKeys = spacingKeys2(spacing);
  const displays = [
    ["block", "block", "Block-level element"],
    ["inline-block", "inline-block", "Inline block-level element"],
    ["inline", "inline", "Inline element"],
  ...
  const colorTokens = colorEntries2(theme.colors);
  for (const [name, shade, value] of colorTokens) {
    const suffix = shade ? `${name}-${shade}` : name;
    add(`text-${suffix}`, "textColor", { color: value }, "colors", `color: ${value}`);
    add(
...
var DEFAULT_EXTRACTOR2 = /[[\w\\:/.-]+/g;
function extractClasses2(content, pattern) {
  const regex = pattern ? new RegExp(pattern, "g") : DEFAULT_EXTRACTOR2;
  const classes = /* @__PURE__ */ new Set();
  for (const chunk of content) {
    if (!chunk) continue;
    for (const match of chunk.matchAll(regex)) {
...
var variantByPrefix2 = new Map(STATE_VARIANTS2.map((v) => [v.prefix, v]));
var CSSGenerator2 = class {
  ...
  constructor(config = {}) {
    ...
    this.config = {
      theme: deepMerge2(defaultTheme2, draft.theme ?? {}),
    ...
    this.variantCfg = this.config.variants;
    this.breakpoints = Object.entries(this.theme.breakpoints ?? {}).filter(([, px]) => typeof px === "number" && px > 0).map(([name, px]) => ({ name, px })).sort((a, b) => a.px - b.px);
    this.utilities = buildUtilityList2(this.theme);
    this.extraBase = {};
    this.extraComponents = {};
    const gen = {
      addUtilities: (utilities, group = "plugin") => {
        for (const [cls, decls] of Object.entries(utilities)) {
          this.utilities.push({
            class: cls,
            group,
            category: GROUP_LABELS2[group] ?? group,
            decls,
            description: `plugin utility ${cls}`
          });
        }
  ...
  getUtilitiesFull(includeVariants = true) {
    ...
    for (const rule of rules) {
      if (rule.group !== lastGroup) {
        lastGroup = rule.group;
        css += `
/* ${GROUP_LABELS2[rule.group] ?? rule.group} */
`;
      }
      css += this.emitRule(rule);
    }
...
`;
  ...
  enabledVariants() {
    return STATE_VARIANTS2.filter((v) => this.variantCfg[v.configKey] !== false);
  }
  enabledBreakpoints() {
    return this.variantCfg.responsive !== false ? this.breakpoints : [];
  }
  ...
  emitRule(rule, prefix = "", media) {
    const variant = prefix ? variantByPrefix2.get(prefix) : void 0;
    let selector;
    if (variant) {
      const full = variant.prefix === prefix ? `${variant.prefix}:${rule.class}` : rule.class;
      const suffix = variant.suffix;
      const ancestor = variant.ancestor;
      selector = ancestor ? `${ancestor} .${escapeClass2(full)}` : `.${escapeClass2(full)}${suffix}`;
    } else if (prefix) {
      selector = `.${escapeClass2(`${prefix}:${rule.class}`)}`;
    } else {
      selector = `.${escapeClass2(rule.class)}`;
    }
    selector = this.wrapSelector(selector);
    const decls = stringifyDecls2(rule.decls, this.config.important === true);
    const body = `${selector} { ${decls}; }`;
    return media ? `  ${body}
` : `${body}
`;
...
`;
    for (const [selector, decls] of Object.entries(this.extraBase)) {
      css += `${this.wrapSelector(selector)} { ${stringifyDecls2(decls, this.config.important === true)}; }
`;
    }
    return css;
  }
...
`;
  ...
  generateJIT(content, _options, internal) {
    ...
    const blocks = [];
    for (const token of found) {
      const { prefixes, base } = splitClass2(token);
      if (prefixes.length > 3) continue;
      const rule = this.utilityByClass.get(base);
      if (!rule) continue;
      if (!this.isGroupEnabled(rule.group)) continue;
      ...
      for (const prefix of prefixes) {
        ...
        const variant = variantByPrefix2.get(prefix);
        if (!variant) {
          ok = false;
          break;
        }
      ...
      const selector = ancestor ? `${ancestor} .${escapeClass2(className)}${suffix}` : `.${escapeClass2(className)}${suffix}`;
      const wrapped = this.wrapSelector(selector);
      const body = `${wrapped} { ${stringifyDecls2(rule.decls, this.config.important === true)}; }
`;
      blocks.push({ media, variantOrder, body: media ? `  ${body}` : body });
    }
    blocks.sort((a, b) => a.variantOrder - b.variantOrder);
  ...
  return {
    ...
    async Once(root) {
      ...
      root.walkAtRules("nakshora", (atRule) => {
        ...
        if (css) {
          atRule.replaceWith(...import_postcss.default.parse(css, { from: void 0 }).nodes);
        } else {
          atRule.remove();
        }
      });
  ...
  return {
    ...
    config(config) {
      ...
      if (postcss2 && typeof postcss2 === "object" && "plugins" in postcss2) {
        postcss2.plugins = [...postcss2.plugins ?? [], plugin];
      } else {
